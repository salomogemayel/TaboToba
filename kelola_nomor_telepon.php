...
<html>
    <head>
        <title>Kelola Nomor Telepon</title>
        <meta charset="utf-8">
                ...
                <div class="container mt-4">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Ubah Nomor Telepon</li>
                        </ol>
                    </nav>
                </div>
            <div class="container my-2" style="max-width: 600px;">

            <h3 class="mb-4 mt-1">Ubah Nomor Telepon</h3>
              <?php
                // Retrieve phone number from the database
                $query = 'SELECT * FROM nomor_telepon';
                $result = $conn->query($query);
                ...
                if (isset($_POST['submit'])) {
                    ...
                    $updateQuery = "UPDATE nomor_telepon SET nomor=?";
                    $stmt = $conn->prepare($updateQuery);
                    $stmt->bind_param("s", $nomor);

                    if ($stmt->execute()) {
                        echo '<div class="alert alert-success mt-3" role="alert">Perubahan berhasil disimpan.</div>';
                        // Update the phone number variable after successful update
                        $phoneNumber['nomor'] = $nomor;
                    } else {
                ...
                ?>
                        <form method="post" enctype="multipart/form-data">
                            <div class="mb-3 mt-3">
                                ...
                                <input type="text" class="form-control" id="exampleFormControlInput1" name="nomor" value="<?php echo $phoneNumber['nomor']; ?>">
                            </div>
